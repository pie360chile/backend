...
from app.backend.db.models import PedagogicalEvaluationClassroomThirdGradeModel
...
_FIELD_NAMES = [
    ...
    "mathematics_11", "mathematics_12", "mathematics_13",
...
def _row_to_dict(r: PedagogicalEvaluationClassroomThirdGradeModel) -> dict:
...
class PedagogicalEvaluationClassroomThirdGradeClass:
    def __init__(self, db: Session):
        self.db = db

    def _to_dict(self, r: PedagogicalEvaluationClassroomThirdGradeModel) -> dict:
        return _row_to_dict(r)

    def get(self, id: int) -> Any:
        try:
            r = self.db.query(PedagogicalEvaluationClassroomThirdGradeModel).filter(PedagogicalEvaluationClassroomThirdGradeModel.id == id).first()
            if r:
    ...
    def get_by_student_id(self, student_id: int) -> Any:
        try:
            r = (
                self.db.query(PedagogicalEvaluationClassroomThirdGradeModel)
                .filter(PedagogicalEvaluationClassroomThirdGradeModel.student_id == student_id)
                .order_by(PedagogicalEvaluationClassroomThirdGradeModel.id.desc())
                .first()
            )
            if r:
                return self._to_dict(r)
    ...
    def store(self, payload: dict) -> Any:
        try:
            ...
            document_type_id = data.get("document_type_id") if data.get("document_type_id") is not None else payload.get("document_type_id", 33)
            if not student_id:
                return {"status": "error", "message": "Falta student_id."}
            data["student_id"] = student_id
            data["document_type_id"] = document_type_id
            row = PedagogicalEvaluationClassroomThirdGradeModel(**{k: v for k, v in data.items() if k in _FIELD_NAMES})
            self.db.add(row)
            self.db.commit()
            self.db.refresh(row)
    ...
    def update(self, id: int, payload: dict) -> Any:
        try:
            row = self.db.query(PedagogicalEvaluationClassroomThirdGradeModel).filter(PedagogicalEvaluationClassroomThirdGradeModel.id == id).first()
            if not row:
                return {"status": "error", "message": "Informe no encontrado."}
            data = _payload_to_row_values(payload)
