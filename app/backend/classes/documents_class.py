...
class DocumentsClass:
    ...
    @staticmethod
    def _generate_paci_progress_state_from_scratch(
        document_id: int,
        paci_data: Dict[str, Any],
        db: Optional[Session] = None,
        output_directory: str = "files/system/students",
    ) -> Dict[str, Any]:
        """Genera PDF «Estado de avance PACI» (documento 21) para una asignatura y período EA."""
        try:
            if not REPORTLAB_AVAILABLE:
                return {
                    "status": "error",
            ...
            student_name_safe = (paci_data.get("student_full_name") or "estudiante").replace(" ", "_")[:40]
            entry_code = (paci_data.get("entry_code") or "EA").replace(" ", "_")
            unique_filename = f"estado_avance_paci_{student_name_safe}_{entry_code}_{uuid.uuid4().hex[:8]}.pdf"
            output_file = Path(output_directory) / unique_filename
            output_file.parent.mkdir(parents=True, exist_ok=True)
            ...
            subject_name = get_value("subject_name")
            if subject_name:
            ...
            sig_rut = get_value("signature_rut")
            sig_secreduc = get_value("signature_secreduc")
            ...
            if DOCX_EXTRA_AVAILABLE:
                try:
                    ...
                    def _set_checkbox_checked(sdt, sdtPr, checked: bool):
                        ...
                        # Informe familia: ☐/☒ visibles en w:t además de w14 (export/PDF/vista web)
                        if content_control_tag_aliases:
                            symbol = "\u2612" if checked else CHK_SYMBOL_UNCHECKED
                        elif checked:
                            symbol = CHK_SYMBOL_CHECKED
                        else:
                            symbol = (
                                ""
                                if checkbox_unchecked_blank
                                else CHK_SYMBOL_UNCHECKED
                            )
