"""Document 38 - Pauta de evaluacion pedagogica - Docente de aula - 1ero Medio. 13 actitud, 13 lengua y literatura, 8 matemática."""
from datetime import datetime
from typing import Any
from sqlalchemy.orm import Session
from app.backend.db.models import PedagogicalEvaluationClassroomFirstGradeSecondaryModel
...
def _row_to_dict(r: PedagogicalEvaluationClassroomFirstGradeSecondaryModel) -> dict:
...
class PedagogicalEvaluationClassroomFirstGradeSecondaryClass:
    def __init__(self, db: Session):
        self.db = db

    def _to_dict(self, r: PedagogicalEvaluationClassroomFirstGradeSecondaryModel) -> dict:
        return _row_to_dict(r)

    def get(self, id: int) -> Any:
        try:
            r = self.db.query(PedagogicalEvaluationClassroomFirstGradeSecondaryModel).filter(PedagogicalEvaluationClassroomFirstGradeSecondaryModel.id == id).first()
            if r:
    ...
    def get_by_student_id(self, student_id: int) -> Any:
        try:
            r = (
                self.db.query(PedagogicalEvaluationClassroomFirstGradeSecondaryModel)
                .filter(PedagogicalEvaluationClassroomFirstGradeSecondaryModel.student_id == student_id)
                .order_by(PedagogicalEvaluationClassroomFirstGradeSecondaryModel.id.desc())
                .first()
            )
            if r:
                return self._to_dict(r)
    ...
    def store(self, payload: dict) -> Any:
        try:
            ...
            row = PedagogicalEvaluationClassroomFirstGradeSecondaryModel(**{k: v for k, v in data.items() if k in _FIELD_NAMES})
            self.db.add(row)
            self.db.commit()
            self.db.refresh(row)
    ...
    def update(self, id: int, payload: dict) -> Any:
        try:
            row = self.db.query(PedagogicalEvaluationClassroomFirstGradeSecondaryModel).filter(PedagogicalEvaluationClassroomFirstGradeSecondaryModel.id == id).first()
            if not row:
                return {"status": "error", "message": "Informe no encontrado."}
            data = _payload_to_row_values(payload)
