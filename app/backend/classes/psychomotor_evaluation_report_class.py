"""Informe de evaluación psicomotriz — persistencia JSON."""
import json
from datetime import datetime
from typing import Any
from sqlalchemy.orm import Session
from app.backend.db.models import PsychomotorEvaluationReportModel


class PsychomotorEvaluationReportClass:
    def __init__(self, db: Session):
        self.db = db

    def _to_dict(self, r: PsychomotorEvaluationReportModel) -> dict:
    ...
    def get(self, id: int) -> Any:
        try:
            r = self.db.query(PsychomotorEvaluationReportModel).filter(PsychomotorEvaluationReportModel.id == id).first()
            if r:
    ...
    def get_by_student_id(self, student_id: int) -> Any:
        try:
            r = (
                self.db.query(PsychomotorEvaluationReportModel)
                .filter(PsychomotorEvaluationReportModel.student_id == student_id)
                .order_by(PsychomotorEvaluationReportModel.id.desc())
                .first()
            )
            if r:
                return self._to_dict(r)
    ...
    def store(self, payload: dict) -> Any:
        try:
            ...
            now = datetime.now()
            row = PsychomotorEvaluationReportModel(
    ...
    def update(self, id: int, payload: dict) -> Any:
        try:
            row = self.db.query(PsychomotorEvaluationReportModel).filter(PsychomotorEvaluationReportModel.id == id).first()
            if not row:
                return {"status": "error", "message": "Informe no encontrado."}
