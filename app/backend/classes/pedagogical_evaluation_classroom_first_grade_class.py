"""Document 31 – Pauta de evaluación pedagógica - Docente de aula - 1º Básico (first grade). Usa columnas normalizadas."""
from datetime import datetime
from typing import Any
from sqlalchemy.orm import Session
from app.backend.db.models import PedagogicalEvaluationClassroomFirstGradeModel

# Columnas que se mapean del payload a la fila (excl. id)
_FIELD_NAMES = [
    ...
    "mathematics_6", "mathematics_7", "mathematics_8", "mathematics_9", "mathematics_10", "mathematics_11",
...
def _row_to_dict(r: PedagogicalEvaluationClassroomFirstGradeModel) -> dict:
...
def _payload_to_row_values(payload: dict) -> dict:
    """Extrae del payload solo las claves que son columnas y convierte fechas."""
    data = {}
    for name in _FIELD_NAMES:
...
class PedagogicalEvaluationClassroomFirstGradeClass:
    def __init__(self, db: Session):
        self.db = db

    def _to_dict(self, r: PedagogicalEvaluationClassroomFirstGradeModel) -> dict:
        return _row_to_dict(r)

    def get(self, id: int) -> Any:
        try:
            r = self.db.query(PedagogicalEvaluationClassroomFirstGradeModel).filter(PedagogicalEvaluationClassroomFirstGradeModel.id == id).first()
            if r:
    ...
    def get_by_student_id(self, student_id: int) -> Any:
        try:
            r = (
                self.db.query(PedagogicalEvaluationClassroomFirstGradeModel)
                .filter(PedagogicalEvaluationClassroomFirstGradeModel.student_id == student_id)
                .order_by(PedagogicalEvaluationClassroomFirstGradeModel.id.desc())
                .first()
            )
            if r:
                return self._to_dict(r)
    ...
    def store(self, payload: dict) -> Any:
        try:
            ...
            document_type_id = data.get("document_type_id") if data.get("document_type_id") is not None else payload.get("document_type_id", 31)
            if not student_id:
                return {"status": "error", "message": "Falta student_id."}
            data["student_id"] = student_id
            data["document_type_id"] = document_type_id
            row = PedagogicalEvaluationClassroomFirstGradeModel(**{k: v for k, v in data.items() if k in _FIELD_NAMES})
            self.db.add(row)
            self.db.commit()
            self.db.refresh(row)
    ...
    def update(self, id: int, payload: dict) -> Any:
        try:
            row = self.db.query(PedagogicalEvaluationClassroomFirstGradeModel).filter(PedagogicalEvaluationClassroomFirstGradeModel.id == id).first()
            if not row:
                return {"status": "error", "message": "Informe no encontrado."}
            data = _payload_to_row_values(payload)
