...
async def checkDocumentExistence(
    ...
    period_year: Optional[int] = Query(
        ...
        ge=2000,
        le=2100,
        description="Año del período escolar (filtra registros en folders)",
    ),
    ...
    try:
        ...
        result = folder_service.check_document_existence(student_id, document_type_id, period_year)
...
async def upload_document(
    ...
    period_year: Optional[int] = Query(
        ...
        ge=2000,
        le=2100,
        description="Año del período escolar",
    ),
    db: Session = Depends(get_db),
):
    """
...
async def list_folders(
    ...
    period_year: Optional[int] = Query(
        ...
        ge=2000,
        le=2100,
        description="Año del período escolar (solo versiones de ese año)",
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    try:
        folder_service = FolderClass(db)
        result = folder_service.get_by_student_and_document(
            student_id, document_id, period_year=str(period_year) if period_year is not None else None
        )
